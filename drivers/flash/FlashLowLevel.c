...
void FlashLowLevel_Init(void)
{
	...
	XBlastLogger(DBG_LVL_DEBUG, DEBUG_FLASH_LOWLEVEL,"Setting LPC address to 0x%08X", LPCFlashadress);
}
...
bool FlashLowLevel_ReadDevice(void)
{
    ...
    while(1)
    {
        ...
        XBlastLogger(DBG_LVL_DEBUG, DEBUG_FLASH_LOWLEVEL,"Read device ID. manf=0x%02X  dev=0x%02X", flashRead.m_bManufacturerId, flashRead.m_bDeviceId);

        if(_MatchDevice(&flashRead))
        {
            // Found Device.
            XBlastLogger(DBG_LVL_DEBUG, DEBUG_FLASH_LOWLEVEL,"Found matching device: %s", flashDevice.flashType.m_szFlashDescription);
            XBlastLogger(DBG_LVL_DEBUG, DEBUG_FLASH_LOWLEVEL,"Additional info: %s", flashDevice.m_szAdditionalErrorInfo);
            XBlastLogger(DBG_LVL_DEBUG, DEBUG_FLASH_LOWLEVEL,"Can Erase/Write: %s", flashDevice.m_fIsBelievedCapableOfWriteAndErase ? "Yes" : "No");
            return true;
        }
        ...
        if(retry == true)
        {
            ...
            XBlastLogger(DBG_LVL_WARN, DEBUG_FLASH_LOWLEVEL,"is28xxxProtocol: %s", is28xxxProtocol ? "true" : "false");
            retry = false;
        }
        else
        {
            // Tried both method. No device found.
            XBlastLogger(DBG_LVL_WARN, DEBUG_FLASH_LOWLEVEL,"Device not found...");
            return false;
        }
    }
}
