...
//Select drive
    ...
    if(n == -1){
    	debugSPIPrint(DBG_LVL_WARN, DEBUG_IDE_DRIVER,"No device at %s position. Halting init.\n", nIndexDrive? "slave" : "master");
    	return 1;
    }
    ...
    if(n == 1) {
        ...
        debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"Drive is ATAPI, surely DVD drive.\n");
        if((cl == 0x14 && ch == 0xEB) || (cl == 0x69 && ch == 0x96)){
            debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"Issuing ATAPI IDENTIFY command.\n");
            if (BootIdeIssueAtaCommand(uIoBase,IDE_CMD_PACKET_IDENTIFY,&tsicp, false)) {
                debugSPIPrint(DBG_LVL_WARN, DEBUG_IDE_DRIVER,"ATAPI IDENTIFY command returned error. Drive not detected. Halting!\n");
                return 1;
            }
            tsaHarddiskInfo[nIndexDrive].m_fAtapi=true;
            debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"Succesfully identified ATAPI device.\n");
        }
        else{
            debugSPIPrint(DBG_LVL_ERROR, DEBUG_IDE_DRIVER,"Magic ATAPI values identifier not valid.\n");
            debugSPIPrint(DBG_LVL_ERROR, DEBUG_IDE_DRIVER,"Unknown device at %s position. Halting init.\n",nIndexDrive? "slave" : "master");
            return 1;
        }
    } 
    else{
        debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"Drive is not ATAPI, so IDE.\n");
        tsaHarddiskInfo[nIndexDrive].m_fAtapi=false;
    }
    ...
    if (tsaHarddiskInfo[nIndexDrive].m_fAtapi) {
     // CDROM/DVD
        debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"ATAPI device specific init routine.\n");
...
#endif
        debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"hd%c:\n", nIndexDrive+'a');
        debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"    %s\n",tsaHarddiskInfo[nIndexDrive].m_szIdentityModelNumber);
        debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"    %s\n", tsaHarddiskInfo[nIndexDrive].m_szIdentityModelNumber);
        debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"    %s\n", tsaHarddiskInfo[nIndexDrive].m_szSerial);
        debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"    %s\n", tsaHarddiskInfo[nIndexDrive].m_szFirmware);
        if (isXBE() == false)
        {
            debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"Reset ATAPI device specific requirement from cold boot.\n");
    ...
    if (!tsaHarddiskInfo[nIndexDrive].m_fAtapi) {       //Drive is HDD (not CD/DVD).
        debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"IDE(HDD) device specific init routine.\n");
        ...
        debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"    %s\n", tsaHarddiskInfo[nIndexDrive].m_szIdentityModelNumber);
        debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"    %s\n", tsaHarddiskInfo[nIndexDrive].m_szFirmware);
        debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"    %u.%uGB\n", ulDriveCapacity1024/1000, ulDriveCapacity1024%1000);
        ...
        if (isXBE() == false) {
            ...
            { 
            	...
            	userPassword[20] = '\0';
            	debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"Drive requires SECURITY_UNLOCK\n");
            	debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"Calculated password: %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X\n",
        ...
        debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"HDD support multiple sector transfers of %u sectors per command\n", tsaHarddiskInfo[nIndexDrive].m_maxBlockTransfer);
        ...
        if((tsaHarddiskInfo[nIndexDrive].m_maxBlockTransfer + 0x0100) != (drive_info[59]&0x01FF)){
            debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"Setting HDD Mutimode sectors to maximum sector per transfer.\n");
            if(BootIdeSetMultimodeSectors(nIndexDrive, tsaHarddiskInfo[nIndexDrive].m_maxBlockTransfer)){
                ...
                debugSPIPrint(DBG_LVL_WARN, DEBUG_IDE_DRIVER,"Unable to change Multimode's sectors per block.\n");
                wait_ms(3000);
                return 1;
            }
        }
        ...
        { 
            ...
            debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"IORDY control bit supported.\n");
            tsaHarddiskInfo[nIndexDrive].m_bIORDY = 1;
        }

        tsaHarddiskInfo[nIndexDrive].m_minPIOcycle = drive_info[67];       //Value in ns
        debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"Minimum PIO cycle for HDD is %u.\n", tsaHarddiskInfo[nIndexDrive].m_minPIOcycle);
...
/*        if(tsaHarddiskInfo[nIndexDrive].m_minPIOcycle <= 120
            ...
            debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"Trying to set PIO mode 1.\n");
            ...
            debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"Trying to set PIO mode 0 (default).\n");
        ...
        if(n){
            debugSPIPrint(DBG_LVL_ERROR, DEBUG_IDE_DRIVER,"BootIdeSetPIOMode:Drive %d: Cannot set PIO mode.\n", nIndexDrive);
...
*/
    ...
    debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"Common init routine for both IDE and ATAPI devices.\n");
    if (drive_info[49] & 0x200) 
    { 
        /* bit 9 of capability word is lba supported bit */
        tsaHarddiskInfo[nIndexDrive].m_bLbaMode = IDE_DH_LBA;
        debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"Drive is accessed by LBA.\n");
    } else {
        tsaHarddiskInfo[nIndexDrive].m_bLbaMode = IDE_DH_CHS;
        debugSPIPrint(DBG_LVL_WARN, DEBUG_IDE_DRIVER,"Drive is accessed by CHS(seriously?).\n");
    }
...
bool driveMasterPasswordUnlock(unsigned uIoBase, int driveId, const char *master_password){
    ...
    unsigned short* drive_info = (unsigned short *)baBuffer;
    tsIdeCommandParams tsicp = IDE_DEFAULT_COMMAND;
    debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"Trying master password unlock on drive %u\n", driveId);
    ...
    debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"Master password is: %s\n", master_password);
...
int BootIdeInit(void)
{
    ...
    debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"IDE init for Master drive\n");
    BootIdeDriveInit(IDE_BASE1, 0);
    debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"IDE init for Slave drive\n");
    ...
    if(tsaHarddiskInfo[0].m_fDriveExists && !tsaHarddiskInfo[0].m_fAtapi)
    {
        ...
        debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"Reading IDE cable information through Master drive\n");
        if(!BootIdeIssueAtaCommand(uIoBase, IDE_CMD_IDENTIFY, &tsicp, false)) 
        {
            unsigned short waBuffer[256];
            BootIdeWaitDataReady(uIoBase);
            debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"Read data back from Master drive\n");
            if(!BootIdeReadData(uIoBase, (unsigned char *)&waBuffer[0], IDE_SECTOR_SIZE)) 
            {
                ...
                {
                    debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"Detected 80-conductors IDE cable\n");
                    tsaHarddiskInfo[0].m_bCableConductors=80;
                }
                else
                {
                    debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"Assum 40-conductors IDE cable\n");
                }

            } else {
                debugSPIPrint(DBG_LVL_ERROR, DEBUG_IDE_DRIVER,"Error getting data from IDENTIFY command\n");
            }
        }
    }
...
#ifndef SILENT_MODE
        printk("UDMA2\n");
        debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"IDE cable limits UDMA support to UDMA2.\n");
#endif
    } else 
    {
        int nAta=0;
        ...
        printk("UDMA%d\n", nAta);
#endif
        debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"UDMA%u supported by Master drive\n", nAta);
    }
    return 0;
}
...
int BootIdeWriteSector(int nDriveIndex, const void * pbBuffer, unsigned int block, unsigned char retry)
{
    ...
    debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER, "Send Write cmd.\n");
    if(BootIdeIssueAtaCommand(uIoBase, ideWriteCommand, &tsicp, false)) 
    {
        printk("ide error %02X...\n", IoInputByte(IDE_REG_ERROR(uIoBase)));
        return 1;
    }
    debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER, "Send Write data.\n");
...
*/
    debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER, "status=%u.\n", status);
    return status;
}
...
int BootIdeWriteMultiple(int nDriveIndex, void * pbBuffer, unsigned int startLBA, unsigned short len, unsigned char retry)
{
    ...
    debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER, "start:%u len:%u retry:%u\n", startLBA, len, retry);
...
*/    
    ...
    if(retry > 0)
        retry -= 1;
    if(status && retry){ //Status set to 1 or 2 means error. retry count must not be 0.
        debugSPIPrint(DBG_LVL_ERROR, DEBUG_IDE_DRIVER, "error! offset:%u retry:%u  remaining:%u\n", startLBA+bufferPtr, retry, remainingLen);
...
int BootIdeFlushCache(int nDriveIndex)
{
    ...
    if(result)
    {
        debugSPIPrint(DBG_LVL_ERROR, DEBUG_IDE_DRIVER, "error : %u\n", result);
    }
...
int HDD_SECURITY_SendATACommand(int nIndexDrive, ide_command_t ATACommand, char * password, bool masterPassword){
    ...
    if(masterPassword && (ATACommand == IDE_CMD_SECURITY_UNLOCK || ATACommand == IDE_CMD_SECURITY_DISABLE)){
        debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"Setting bit for Master Password usage.\n");
    	//Set master password flag
    	ide_cmd_data[0]|=0x01;
    }
    ...
    if(masterPassword && tsaHarddiskInfo[nIndexDrive].m_masterPassSupport != 0xFFFF && ATACommand == IDE_CMD_SECURITY_SET_PASSWORD){
        ide_cmd_data[34] = 1;
        debugSPIPrint(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"HDD support MasterPassword Index. Setting this to \"1\".\n");
    }
    ...
    // check that we are unlocked
    debugSPIPrint(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"Security change command sent. Refreshing drives properties.\n");
    if(BootIdeIssueAtaCommand(uIoBase, IDE_CMD_IDENTIFY, &tsicp, false)) 
    {
        return 1;
    }
