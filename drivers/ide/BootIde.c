...
//    wait_us(1);
    ...
    do {
        if (((IoInputByte(IDE_REG_STATUS(uIoBase)) & 0x88) == 0x08)){       //DRQ bit raised and BSY bit cleared.
            if(IoInputByte(IDE_REG_STATUS(uIoBase)) & 0x01){
                XBlastLogger(DEBUG_IDE_DRIVER,"Error bit raised.\n");
    ...
    if(IoInputByte(IDE_REG_STATUS(uIoBase)) & 0x01) {
        XBlastLogger(DEBUG_IDE_DRIVER,"Error bit raised after timed out.\n");
        return 2;                //ERR bit raised.
    }
    XBlastLogger(DEBUG_IDE_DRIVER,"Timeout error.\n");
    ...
    tsIdeCommandParams * params, bool skipFirstWait)
{
    ...
    if(!skipFirstWait)
    {
        ...
        if(n == -1)    {// as our command may be being used to clear the error, not a good policy to check too closely!
            XBlastLogger(DEBUG_IDE_DRIVER,"Error on BootIdeIssueAtaCommand wait 1: ret=%d, error %02X\n", n, IoInputByte(IDE_REG_ERROR(uIoBase)));
            return n;
        }
    }
...
//        printk("\n      error on BootIdeIssueAtaCommand wait 3: ret=%d, error %02X\n", n, IoInputByte(IDE_REG_ERROR(uIoBase)));
        XBlastLogger(DEBUG_IDE_DRIVER,"Error on BootIdeIssueAtaCommand wait 2: ret=%d, error %02X\n", n, IoInputByte(IDE_REG_ERROR(uIoBase)));
        return n;
    }
...
int BootIdeReadData(unsigned uIoBase, void * buf, size_t size)
{
    ...
    if (BootIdeWaitDataReady(uIoBase)) {
        XBlastLogger(DEBUG_IDE_DRIVER,"Data not ready..\n");
        return 1;
    }
    ...
    if(IoInputByte(IDE_REG_STATUS(uIoBase)) & 0x01) {
        XBlastLogger(DEBUG_IDE_DRIVER,"Ended with an error.\n");
        return 2;
    }
    return 0;
}
...
int BootIdeWriteData(unsigned uIoBase, const void * buf, unsigned int size)
{
    ...
    n=BootIdeWaitNotBusy(uIoBase);
    if(n) {
        XBlastLogger(DEBUG_IDE_DRIVER,"Waiting for good status reg returned error : %d\n", n);
        return n;
    }
...
//Select drive
    ...
    if(n == -1){
    	XBlastLogger(DBG_LVL_WARN, DEBUG_IDE_DRIVER,"No device at %s position. Halting init.\n", nIndexDrive? "slave" : "master");
    	return 1;
    }
    ...
    if(n == 1) {
        ...
        if((cl == 0x14 && ch == 0xEB) || (cl == 0x69 && ch == 0x96)){
            ...
            XBlastLogger(DBG_LVL_ERROR, DEBUG_IDE_DRIVER,"Unknown device at %s position. Halting init.\n",nIndexDrive? "slave" : "master");
            return 1;
        }
    } 
    else{
...
#endif
        XBlastLogger(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"hd%c:\n", nIndexDrive+'a');
        XBlastLogger(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"    %s\n",tsaHarddiskInfo[nIndexDrive].m_szIdentityModelNumber);
        XBlastLogger(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"    %s\n", tsaHarddiskInfo[nIndexDrive].m_szIdentityModelNumber);
        XBlastLogger(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"    %s\n", tsaHarddiskInfo[nIndexDrive].m_szSerial);
        XBlastLogger(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"    %s\n", tsaHarddiskInfo[nIndexDrive].m_szFirmware);
        if (isXBE() == false)
    ...
    if (!tsaHarddiskInfo[nIndexDrive].m_fAtapi) {       //Drive is HDD (not CD/DVD).
        ...
        XBlastLogger(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"    %s\n", tsaHarddiskInfo[nIndexDrive].m_szIdentityModelNumber);
        XBlastLogger(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"    %s\n", tsaHarddiskInfo[nIndexDrive].m_szFirmware);
        XBlastLogger(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"    %u.%uGB\n", ulDriveCapacity1024/1000, ulDriveCapacity1024%1000);
        ...
        XBlastLogger(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"HDD support multiple sector transfers of %u sectors per command\n", tsaHarddiskInfo[nIndexDrive].m_maxBlockTransfer);
        ...
        XBlastLogger(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER,"Minimum PIO cycle for HDD is %u.\n", tsaHarddiskInfo[nIndexDrive].m_minPIOcycle);
...
/*        if(tsaHarddiskInfo[nIndexDrive].m_minPIOcycle <= 120
        ...
        if(n){
            XBlastLogger(DBG_LVL_ERROR, DEBUG_IDE_DRIVER,"BootIdeSetPIOMode:Drive %d: Cannot set PIO mode.\n", nIndexDrive);
...
bool driveMasterPasswordUnlock(unsigned uIoBase, int driveId, const char *master_password){
    ...
    unsigned short* drive_info = (unsigned short *)baBuffer;
    tsIdeCommandParams tsicp = IDE_DEFAULT_COMMAND;
    XBlastLogger(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"Trying master password unlock on drive %u\n", driveId);
...
int BootIdeInit(void)
{
    ...
    } else 
    {
        ...
        printk("UDMA%d\n", nAta);
#endif
        XBlastLogger(DBG_LVL_INFO, DEBUG_IDE_DRIVER,"UDMA%u supported by Master drive\n", nAta);
    }
    return 0;
}
...
*/
    XBlastLogger(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER, "status=%u.\n", status);
    return status;
}
...
int BootIdeWriteMultiple(int nDriveIndex, void * pbBuffer, unsigned int startLBA, unsigned short len, unsigned char retry)
{
    ...
    XBlastLogger(DBG_LVL_DEBUG, DEBUG_IDE_DRIVER, "start:%u len:%u retry:%u\n", startLBA, len, retry);
...
*/    
    ...
    if(retry > 0)
        retry -= 1;
    if(status && retry){ //Status set to 1 or 2 means error. retry count must not be 0.
        XBlastLogger(DBG_LVL_ERROR, DEBUG_IDE_DRIVER, "error! offset:%u retry:%u  remaining:%u\n", startLBA+bufferPtr, retry, remainingLen);
...
int BootIdeFlushCache(int nDriveIndex)
{
    ...
    if(result)
    {
        XBlastLogger(DBG_LVL_ERROR, DEBUG_IDE_DRIVER, "error : %u\n", result);
    }
