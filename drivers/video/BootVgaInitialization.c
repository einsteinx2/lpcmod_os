...
void BootVgaInitializationKernelNG(CURRENT_VIDEO_MODE_DETAILS * pvmode)
{
    ...
    XBlastLogger(DBG_LVL_INFO, DEBUG_VIDEO_DRIVER, "Detected Video Standard:%s", tv_encoding == TV_ENC_NTSC ? "NTSC" : "PAL");
    DetectVideoEncoder();
    XBlastLogger(DBG_LVL_INFO, DEBUG_VIDEO_DRIVER, "Detected Video Encoder: %s", VideoEncoderName());
    ...
    XBlastLogger(DBG_LVL_INFO, DEBUG_VIDEO_DRIVER, "Detected Video Cable: %s", AvCableName());
    ...
    {
        XBlastLogger(DBG_LVL_INFO, DEBUG_VIDEO_DRIVER, "Screen format set to 16:9");
        pvmode->m_nVideoModeIndex=VIDEO_MODE_1024x576;
    }
    else // 4:3 TV
    {
        XBlastLogger(DBG_LVL_INFO, DEBUG_VIDEO_DRIVER, "Screen format set to 4:3");
