...
void WriteLCDInit(struct Disp_controller *xLCD){
	...
	xLCD->Command(xLCD,DISP_FUNCTION_SET | DISP_N_FLAG | DISP_RE_FLAG);	//2 lines and 5x8 dots character resolution.
	xLCD->Command(xLCD,DISP_SEGRAM_SET);			//Display OFF, Cursor OFF, Cursor blink OFF.
	xLCD->Command(xLCD,DISP_EXT_CONTROL | DISP_NW_FLAG);
	xLCD->Command(xLCD,DISP_FUNCTION_SET | DISP_N_FLAG);	//Entry mode,Increment cursor, shift right
	...
	xLCD->Command(xLCD,DISP_DDRAM_SET);			//Set to position 0x00	
...
void WriteLCDLine1(struct Disp_controller *xLCD, bool centered, char *lineText){
	...
	//Place cursor
	WriteLCDSetPos(xLCD,0,0);	// Write to first line
...
void WriteLCDLine2(struct Disp_controller *xLCD, bool centered, char *lineText){
	...
	//Place cursor
	WriteLCDSetPos(xLCD,0,1);	// Write to second line
...
void WriteLCDLine4(struct Disp_controller *xLCD, bool centered, char *lineText){
	...
	//Place cursor
	WriteLCDSetPos(xLCD,0,3);	// Write to fourth line
