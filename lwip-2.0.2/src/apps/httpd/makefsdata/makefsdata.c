...
int main(int argc, char *argv[])
{
  ...
  if(GETCWD(appPath, MAX_PATH_LEN) == NULL)
  {
...
void process_file_data(FILE* data_file, u8_t* file_data, size_t file_size)
{
  ...
  for (i = 0; i < file_size; i++) {
    ...
    sprintf(&file_buffer_c[off], "0x%2.2x,", file_data[i]);
    off += 5;
                    ...
                    u16_t hdr_len, u16_t hdr_chksum, const u8_t* file_data, size_t file_size)
{
  ...
  for (offset = hdr_len; ; offset += len) {
    ...
    fprintf(struct_file, "{%d, 0x%04x, %lu}," NEWLINE, offset, chksum, len);
    i++;
  }
                           ...
                           u16_t *http_hdr_chksum, u8_t provide_content_len, int is_compressed)
{
  ...
  fprintf(data_file, NEWLINE "/* \"%s\" (%lu bytes) */" NEWLINE, cur_string, cur_len);
  ...
  if (provide_content_len) {
    ...
    cur_len = strlen(cur_string);
    fprintf(data_file, NEWLINE "/* \"%s%d\r\n\" (%lu+ bytes) */" NEWLINE, cur_string, content_len, cur_len+2);
    written += file_put_ascii(data_file, cur_string, cur_len, &i);
    if (precalcChksum) {
      memcpy(&hdr_buf[hdr_len], cur_string, cur_len);
      hdr_len += cur_len;
  ...
  if (provide_last_modified) {
    ...
    cur_len = strlen(cur_string);
    fprintf(data_file, NEWLINE "/* \"%s\"\r\n\" (%lu+ bytes) */" NEWLINE, cur_string, cur_len+2);
    written += file_put_ascii(data_file, cur_string, cur_len, &i);
    if (precalcChksum) {
      memcpy(&hdr_buf[hdr_len], cur_string, cur_len);
      hdr_len += cur_len;
