...
typedef struct {
    ...
    unsigned rsrvd:24;    /* 3: */
    unsigned int size;        /* 4: size in byte */
    unsigned len:24;    /* 3: length in unit of fsblksize */
    unsigned addr1:8;    /* 1: address in unit of fsblksize */
    unsigned int addr2;        /* 4: address in unit of fsblksize */
} dxd_t;            /* - 16 - */
 ...
 */
typedef struct dasd {
    unsigned char thresh;        /* Alert Threshold (in percent) */
    unsigned char delta;        /* Alert Threshold delta (in percent)   */
    unsigned char rsrvd1;
    unsigned char limit_hi;        /* DASD limit (in logical blocks)       */
    unsigned int limit_lo;        /* DASD limit (in logical blocks)       */
    unsigned char rsrvd2[3];
    unsigned char used_hi;        /* DASD usage (in logical blocks)       */
    unsigned int used_lo;        /* DASD usage (in logical blocks)       */
} dasd_t;
...
{
    unsigned int s_magic;        /* 4: magic number */
    unsigned int s_version;        /* 4: version number */
    ...
    unsigned int s_agsize;        /* 4: allocation group size in aggr. blocks */

    unsigned int s_flag;        /* 4: aggregate attributes:
                 *    see jfs_filsys.h
                 */
    unsigned int s_state;        /* 4: mount/unmount/recovery state: 
                 *    see jfs_filsys.h
                 */
    ...
    pxd_t s_aim2;        /* 8: first extent of secondary
                 *    aggregate inode map
                 */
    unsigned int s_logdev;        /* 4: device address of log */
    ...
    char s_attach;        /* 1: VFS: flag: set when aggregate is attached
                 */
    unsigned char rsrvd4[7];        /* 7: reserved - set to 0 */
 ...
 */
typedef struct {
    unsigned int inumber;        /* 4: 4-byte aligned */
    s8 next;        /* 1: */
    unsigned char namlen;        /* 1: */
    UniChar name[11];    /* 22: 2-byte aligned */
    unsigned int index;        /* 4: index into dir_table */
} ldtentry_t;            /* (32) */
...
typedef struct dir_table_slot {
    ...
    unsigned char flag;    /* 1: 0 if free */
    unsigned char slot;    /* 1: slot within leaf page of entry */
    unsigned char addr1;    /* 1: upper 8 bits of leaf page address */
    unsigned int addr2;    /* 4: lower 32 bits of leaf page address -OR-
...
typedef union {
    struct {
        ...
        unsigned int idotdot;    /* 4: parent inode number */
...
typedef union {
    struct {
        ...
        unsigned char maxslot;    /* 1: number of slots in page slot[] */
        s8 stblindex;    /* 1: slot index of start of stbl */
        unsigned char rsrvd[2];    /* 2: */
...
typedef struct xad {
    ...
    unsigned int off2;        /* 4: offset in unit of fsblksize */
    unsigned len:24;    /* 3: length in unit of fsblksize */
    unsigned addr1:8;    /* 1: address in unit of fsblksize */
    unsigned int addr2;        /* 4: address in unit of fsblksize */
} xad_t;            /* (16) */
...
struct dinode {
     ...
     */
    unsigned int di_inostamp;    /* 4: stamp to show inode belongs to fileset */
    s32 di_fileset;        /* 4: fileset number */
    unsigned int di_number;        /* 4: inode number, aka file serial number */
    unsigned int di_gen;        /* 4: inode generation number */
    ...
    unsigned int di_nlink;        /* 4: number of links to the object */

    unsigned int di_uid;        /* 4: user id of owner */
    unsigned int di_gid;        /* 4: group id of owner */

    unsigned int di_mode;        /* 4: attribute, format and permission */
...
#define di_DASD        di_dtroot.header.DASD
        ...
        struct {
            union {
                unsigned char _data[96];        /* 96: unused */
                struct {
                    void *_imap;    /* 4: unused */
                    unsigned int _gengen;    /* 4: generator */
                } _imap;
            } _u1;                /* 96: */
#define di_gengen    u._file._u1._imap._gengen
            ...
            union {
                xtpage_t _xtroot;
                struct {
                    unsigned char unused[16];    /* 16: */
                    dxd_t _dxd;    /* 16: */
                    union {
                        unsigned int _rdev;    /* 4: */
                        unsigned char _fastsymlink[128];
                    } _u;
                    unsigned char _inlineea[128];
                } _special;
            } _u2;
        } _file;
...
typedef struct {
    ...
    /* (72) */
    unsigned char pad[1976];        /* 1976: pad to 2048 bytes */
    /* allocation bit map: 1 bit per inode (0 - free, 1 - allocated) */
    unsigned int wmap[EXTSPERIAG];    /* 512: working allocation map  */
    unsigned int pmap[EXTSPERIAG];    /* 512: persistent allocation map */
=======
